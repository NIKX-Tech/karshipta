...
`libs/ward/include/ward_connection.h`, `libs/ward/src/ward_connection.cpp`

## Overview

`WardConnection` owns the MAVLink connection lifecycle for exactly **one**
ward: opening it, retrying it, reporting live link state, and closing it.
It is a thin, RAII-safe handle to a `mavsdk::System`, built on top of a
`mavsdk::Mavsdk` core that is **shared** across every ward in the fleet.
It does not perform telemetry or send commands; it hands out the `System` it
discovered so other classes can do that.

## Responsibilities

- Register/deregister one ward's connection URL on a shared `Mavsdk` core.
- Discover that ward's `mavsdk::System` by MAVLink system id and hold it
...
- **Ward identity beyond the MAVLink system id** (`ward_id`,
  human-facing labels like `"sitl-1"`, and the `ward_id -> system_id`
  mapping itself). Assigned and owned by whoever constructs a fleet of these
  (`WardManager`), which passes the resolved system id into the
  constructor.
...
| Member | Behavior |
|---|---|
| `WardConnection(shared_ptr<Mavsdk>, const string& connection_url, optional<uint32_t> expected_system_id = nullopt)` | Stores the shared core and validates `connection_url`. Does not open a connection. `expected_system_id` is the MAVLink system id (`WardInfo.mavlink_system_id`) this connection must bind to; omit only for the single-ward M1 case. |
| `static shared_ptr<Mavsdk> create_shared_core()` | Builds a `Mavsdk` core configured `ComponentType::GroundStation`. Call once per fleet; pass the result to every `WardConnection`. |
...
| `shared_ptr<System> get_system() const` | The discovered ward handle; `nullptr` if not connected. |
...
WardConnection a(core, "udp://:14540", 1);
WardConnection b(core, "udp://:14541", 2);
WardConnection c(core, "udp://:14542", 3);
```

All three `WardConnection`s' `mavsdk_` members point at the same
...
many wards on one link: every message carries a `system_id`/
`component_id` in its header specifically so one listener can distinguish
many wards. MAVSDK's `Mavsdk::systems()` returns a *vector*, and
`gateway/BRIEF.md`'s `WardManager` is specified as owning N `Ward`
...
N-ward fleet, for no benefit MAVSDK's own design already gives you for
free.

## Identity: system id, not discovery order

`connect()` never relies on "whichever `System` the shared core happens to
have seen" to decide which ward it just bound to. If `expected_system_id`
...
`expected_system_id` is optional so the single-ward M1 program (one
`WardConnection`, no config file yet) can keep using bare
`first_autopilot()`. As soon as a config file exists mapping
`ward_id -> mavlink_system_id`, every constructed `WardConnection`
...
~WardConnection();  // calls disconnect()
```
...
- **Not thread-safe.** No internal synchronization guards `system_`,
  `connection_added_`, or `connection_handle_`. A single `WardConnection`
...
- **The caller must keep the shared `Mavsdk` core alive** at least as long as
  every `WardConnection` built on top of it: the class holds a
  `shared_ptr` but does not create or extend the core's lifetime beyond
  normal reference counting.

## Automated tests

`gateway/tests/ward/ward_connection_test.cpp` (GoogleTest, built with
...
To exercise the system-id path directly, construct a `WardConnection`
with the SITL's system id (PX4 SITL defaults to system id 1):

```cpp
WardConnection ward(WardConnection::create_shared_core(), "udp://:14540", 1);
```
