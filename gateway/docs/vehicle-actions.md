...
`libs/vehicle/include/vehicle_actions.h`, `libs/vehicle/src/vehicle_actions.cpp`

## Overview

`VehicleActions` wraps MAVSDK's `Action` plugin for exactly **one**
already-connected vehicle. Its surface is deliberately narrow: every public
method exists because a field in `Command.action`
(`proto/karshipta/v1/command.proto`) needs it, and no method exists that a
field does not back yet. It reads through a `VehicleConnection&` rather than
...
- Lazily bind a `mavsdk::Action` plugin to `VehicleConnection::get_system()`
...
- **Connection lifecycle** (connect/retry/disconnect/link state). Owned by
  `VehicleConnection`; `VehicleActions` only reads `is_connected()`,
...
| Member | Behavior |
|---|---|
| `explicit VehicleActions(VehicleConnection&)` | Binds to a connection. Does not create the `Action` plugin yet; that happens lazily on first use. |
| `Result arm() const` | Implements `ArmCommand`. Arms the vehicle. |
| `Result disarm() const` | Implements `DisarmCommand{force: false}`. The autopilot rejects this while flying; only a landed vehicle actually disarms. |
...
| `Result kill() const` | Implements `DisarmCommand{force: true}`. Disarms immediately regardless of landed state; the vehicle falls out of the sky if used while flying. The M3 executor must gate this on `force` and must never call it for a plain `DisarmCommand`. |
| `Result set_current_speed(float) const` | Backs `GotoCommand.speed_m_s`. Ephemeral; not stored on the vehicle. |
...
`kill()` is dangerous enough (the vehicle free-falls if this is sent while
...
- **Not thread-safe**, consistent with `VehicleConnection` and
  `TelemetryInfo`.
- **Not wired to the `Transport` yet.** Nothing in `main.cpp` constructs a
  `VehicleActions` or calls it; it becomes reachable only once the M3
...
by `gateway/tests/vehicle/vehicle_connection_test.cpp`: `arm()`/`takeoff()`/
...
`vehicle` library: connect, `TelemetryInfo::check_drone_health()`, then
...
[info] vehicle not ready to arm, waiting on:
[info]   - GPS fix
...
`landing_state()` polling, trimmed here for length.) The vehicle armed,
