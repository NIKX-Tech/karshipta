...
`libs/ward/include/mission_importer.h`, `libs/ward/src/mission_importer.cpp`
...
the exact same `WardMission::enqueue_upload()` a console-editor-authored
...
tracks progress. It reads through a `WardConnection&` rather than owning
...
- Lazily bind a `mavsdk::MissionRaw` plugin to `WardConnection::get_system()`
...
- **Uploading, starting, pausing, or tracking the imported mission.**
  `WardMission`'s job, via the exact same `Mission` type this class
  produces; `MissionImporter` never references `WardMission`.
- **Mission-level validation** (non-empty `ward_id`, RTL only as the last
  item). `WardMission::enqueue_upload()`'s own `validate_mission()`
...
| Member | Behavior |
|---|---|
| `explicit MissionImporter(WardConnection&)` | Binds to a connection. Does not create the `MissionRaw` plugin yet; that happens lazily on first use. |
...
mission editor can produce (see `ward-mission.md`'s `translate()` design).
...
silently send the ward to the wrong altitude, so it's rejected instead.
...
either. `import()` synthesizes one (`"imported-" + ward_id + "-" +
...
- **Requires a successful `WardConnection::connect()` before parsing can
  run**, even though the parse itself is local text processing with no
  MAVLink round trip to the ward: MAVSDK's `MissionRaw` plugin must still
  be constructed against a discovered `System`. `import()` returns `nullopt`
  with a "ward not connected" reason otherwise, checked only after the
  format itself is confirmed valid (so a malformed request fails without
  needing a connection at all).
- **Does not rebind across reconnects**, same as `TelemetryInfo`/`WardMission`.
- **Only mapping-level validation.** A `Mission` returned by `import()` is
  not guaranteed to pass `WardMission::enqueue_upload()`'s own
...
`gateway/tests/ward/mission_importer_test.cpp`, against the same
deliberately-unconnected-`WardConnection` strategy used for
`WardActions`/`WardMission`: an unspecified format rejects without
...
functions don't need a live ward at all, just a constructed plugin
...
accepted gap already documented for `WardActions`/`WardMission`.
