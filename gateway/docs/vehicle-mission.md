...
`libs/vehicle/include/vehicle_mission.h`, `libs/vehicle/src/vehicle_mission.cpp`

## Overview

`VehicleMission` wraps MAVSDK's `Mission` plugin for exactly **one**
already-connected vehicle: uploading a mission, starting/pausing it,
reporting its progress, and looping `repeat_count` extra passes (BRIEF.md
M5). It reads through a `VehicleConnection&` rather than owning a `System`
...
reference to `VehicleActions` or `CommandExecutor` at all: it only exposes
state (`get_progress()`, `take_pending_return_to_launch()`,
`take_upload_result()`) for `VehicleManager`'s existing shared publish tick
to poll, the same way it already polls `TelemetryInfo`.

## Responsibilities

- Lazily bind a `mavsdk::Mission` plugin to `VehicleConnection::get_system()`
...
- **Connection lifecycle.** Owned by `VehicleConnection`; `VehicleMission`
  only reads `is_connected()`, `get_system()`, and `get_mavsdk()` from it.
- **Non-mission commands** (arm/disarm/takeoff/land/rtl/goto). `VehicleActions`'
  job, using MAVSDK's `Action` plugin against the same `VehicleConnection`.
...
- **Publishing to the wire.** Translating `get_progress()`'s result and the
  two pending-flag accessors into `Envelope` frames is `VehicleManager`'s
...
| Member | Behavior |
|---|---|
| `explicit VehicleMission(VehicleConnection&)` | Binds to a connection. Does not create the `Mission` plugin yet; that happens lazily on first use. |
| `~VehicleMission()` | Unsubscribes the mission-progress callback if the plugin was ever created. |
...
`CommandExecutor`'s own queue/worker split (`libs/vehicle/src/command_executor.cpp`):
...
void VehicleMission::enqueue_download() {
...
never an actual on-vehicle mission item (see the RTL-item design section
...
plan. A separate free function in `vehicle_mission.cpp`'s anonymous
namespace, `validate_mission()`, runs first and rejects what `translate()`
cannot: an empty `vehicle_id`, an empty `items` list, or a
...
out-parameter; `run_upload_worker()` stores that as
...
for `VehicleManager` to act on explicitly (via `VehicleActions`) — an
...
upload succeeds (`run_upload_worker()`, not only from the first
...
- `queue_mutex_`/`queue_changed_`/`upload_queue_` are separate from
...
- `state_mutex_` guards every piece of progress/pass/interrupt state,
  written from both `run_upload_worker()`'s thread and `handle_progress()`'s
  ...
  `VehicleManager`'s publish tick or an interrupting command runs on).

## RAII and ownership rules

```cpp
explicit VehicleMission(VehicleConnection& connection);
~VehicleMission();  // unsubscribes mission-progress if the plugin was ever created
```
...
- **No default construction, no copy/move declared** (implicitly deleted by
  ...
  `std::mutex` members): mirrors `VehicleActions`/`TelemetryInfo`.
- **Does not own `connection_`.** The caller must keep the referenced
  `VehicleConnection` alive for at least as long as this `VehicleMission`.
- **Destructor unsubscribes before `upload_worker_` joins.** The explicit
  destructor body runs first (unsubscribing the progress callback), then
  `upload_worker_`'s own `jthread` destructor requests a stop and joins,
...
`gateway/tests/vehicle/vehicle_mission_test.cpp`, against the same
deliberately-unconnected-`VehicleConnection` strategy `command_executor_test.cpp`
...
(the pattern `vehicle_connection_test.cpp` uses) does not implement the
mission upload protocol, so it would not give deterministic results. Same
accepted gap `command_executor_test.cpp` documents for `VehicleActions`.
...
followed by progress-driven log lines as the vehicle flies it, and, if the
...
`take_pending_return_to_launch()` directly; `VehicleManager` does not yet
