#ifndef KARSHIPTA_GATEWAY_WARD_MISSION_H
#define KARSHIPTA_GATEWAY_WARD_MISSION_H
...
// Wraps MAVSDK's Mission plugin against exactly one connected ward.
...
// reference to WardActions or CommandExecutor: it only ever exposes state
// for WardManager to poll (get_progress(), take_pending_return_to_launch(),
// take_upload_result(), take_download_result()) from its existing shared
// publish tick, the same way it already reaches across a ManagedWard's
// other owned members. This keeps WardActions' single-caller-thread
// invariant intact (see gateway/docs/ward-mission.md) and avoids a
...
class WardMission {
   public:
    ...
    WardMission(const WardMission&) = delete;
    WardMission& operator=(const WardMission&) = delete;
    WardMission(WardMission&&) = delete;
    WardMission& operator=(WardMission&&) = delete;
    ...
    // whatever thread received the frame). Resets this ward's repeat-pass
    ...
    // uploaded to the ward. worker_ calls Mission::download_mission()
    ...
    // translate() at upload time was never an actual on-ward mission
    // item, so a downloaded mission never reconstructs one, even if this
    // same WardMission uploaded it with ends_with_return_to_launch_ set.
    ...
    // WardManager's existing shared publish loop the same way it already
    ...
    // force-disarm) against this ward while a mission may be active.
    ...
    // completes. WardManager's publish tick checks this for every ward
    // and, when true, enqueues a synthetic ReturnToLaunchCommand on that
    // ward's CommandExecutor, exactly as if the console had sent one.
    ...
    // call. WardManager's publish tick checks this for every ward and
   ...
   private:
    ...
    // notify_interrupted() (WardManager's publish thread and whichever
    ...
    // is_download is true) download whatever is currently on the ward.
    ...
    // Result through unchanged, mirroring WardActions::log_result.
    ...
    // ward's current mission and records the outcome in
    ...
    // the ward where the mission ended (final pass, no trailing RTL item).
