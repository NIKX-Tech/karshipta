...
class TelemetryInfo {
   public:
    ...
    // Returns the ward's current altitude relative to takeoff, in meters.
    [[nodiscard]] float get_relative_altitude_m() const;
    // Returns the ward's current estimated battery remaining, in percent (0 to 100).
    [[nodiscard]] float get_battery_remaining_percent() const;
    // Returns the ward's current global position (lat/lon/altitude). Default-constructed
    // (NaN fields) if the Telemetry plugin isn't available yet.
    [[nodiscard]] mavsdk::Telemetry::Position get_position() const;
    // Returns the ward's current velocity in the NED frame, in m/s.
    [[nodiscard]] mavsdk::Telemetry::VelocityNed get_velocity_ned() const;
    // Returns the ward's current heading, in degrees true north (0 to 360).
    [[nodiscard]] float get_heading_deg() const;
    // Returns the ward's current full battery reading (voltage + remaining percent).
    [[nodiscard]] mavsdk::Telemetry::Battery get_battery() const;
    // Returns the ward's current GPS fix type and satellite count.
    [[nodiscard]] mavsdk::Telemetry::GpsInfo get_gps_info() const;
    // Returns the ward's current raw GPS reading (adds hdop/vdop over get_gps_info()).
    [[nodiscard]] mavsdk::Telemetry::RawGps get_raw_gps() const;
    // Returns the ward's current flight mode.
    [[nodiscard]] mavsdk::Telemetry::FlightMode get_flight_mode() const;
    // True if the ward is currently armed.
    [[nodiscard]] bool is_armed() const;
    // True if the ward is currently airborne.
   ...
   private:
    ...
    // cleared by unsubscribe_*() and the destructor). Mirrors WardActions::init_mutex_.
