...
// progress; that stays WardMission's job against the Mission plugin, and
// this class's output is meant to be handed straight to
// WardMission::enqueue_upload() exactly like a console-editor-authored
// Mission would be. One instance per ward, same lazy-bind-to-a-connection
// pattern as WardActions/TelemetryInfo/WardMission.
class MissionImporter {
   public:
    ...
    explicit MissionImporter(WardConnection& connection);
    ...
    // WardMission::enqueue_upload(). Returns nullopt (with a
    ...
    // all); mission-level rules (non-empty ward_id, RTL only as the last
    // item) are WardMission::enqueue_upload()'s job via its own
    // validate_mission(), not duplicated here.
    //
    // Blocking, but on local text parsing, not a MAVLink round trip to the
    // ward; still requires a connected WardConnection, since MAVSDK's
    // MissionRaw plugin must be constructed against a discovered System even
    // though the parse itself doesn't need the ward armed, flying, or
    // otherwise reachable.
