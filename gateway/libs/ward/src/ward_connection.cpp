...
WardConnection::WardConnection(std::shared_ptr<mavsdk::Mavsdk> mavsdk,
...
WardConnection::~WardConnection() { disconnect(); }

std::shared_ptr<mavsdk::Mavsdk> WardConnection::create_shared_core() {
    return std::make_shared<mavsdk::Mavsdk>(
        mavsdk::Mavsdk::Configuration{mavsdk::ComponentType::GroundStation});
}

void WardConnection::set_connection_url(const std::string& connection_url) {
    this->connection_url_ = validate_connection_url(connection_url);
}

std::string WardConnection::get_connection_url() const { return this->connection_url_; }
std::string WardConnection::validate_connection_url(const std::string& connection_url) {
...
std::optional<std::shared_ptr<mavsdk::System>> WardConnection::find_system(
...
std::optional<std::shared_ptr<mavsdk::System>> WardConnection::wait_for_system(
    uint32_t system_id) const {
...
WardConnection::ConnectResult WardConnection::connect() {
...
bool WardConnection::connect_with_retry(const std::stop_token& stop_token,
...
void WardConnection::disconnect() {
    std::lock_guard lock(system_mutex_);
    if (!system_) {
        return;
    }
...
bool WardConnection::is_connected() const { return link_state() == LinkState::kConnected; }

WardConnection::LinkState WardConnection::link_state() const {
    std::lock_guard lock(system_mutex_);
    if (!system_) {
...
std::shared_ptr<mavsdk::System> WardConnection::get_system() const {
    std::lock_guard lock(system_mutex_);
    return system_;
}

std::shared_ptr<mavsdk::Mavsdk> WardConnection::get_mavsdk() const { return mavsdk_; }

mavsdk::System::IsConnectedHandle WardConnection::subscribe_connection_state(
...
void WardConnection::unsubscribe_connection_state(mavsdk::System::IsConnectedHandle handle) {
    std::lock_guard lock(system_mutex_);
    if (!system_) {
        return;
    }
