...
// since guessing wrong here means the ward flies to the wrong altitude.
...
std::string synthesize_mission_id(const std::string& ward_id) {
    const auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
    return "imported-" + ward_id + "-" + std::to_string(now_ms);
}
...
MissionImporter::MissionImporter(WardConnection& connection) : connection_(connection) {}
...
std::pair<std::optional<karshipta::v1::Mission>, std::string> MissionImporter::import(
    ...
    if (!ensure_mission_raw()) {
        return {std::nullopt, "mission file import failed: ward not connected"};
    }
    ...
    if (result != mavsdk::MissionRaw::Result::Success) {
        spdlog::error("mission file import rejected for {}: {}", upload.ward_id(),
                      result_name(result));
    ...
    mission.set_mission_id(synthesize_mission_id(upload.ward_id()));
    mission.set_ward_id(upload.ward_id());
