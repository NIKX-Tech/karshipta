...
// fixed by the header): an empty vehicle_id, an empty item list, or an RTL
...
std::optional<std::string> validate_mission(const karshipta::v1::Mission& mission) {
    if (mission.vehicle_id().empty()) {
        return "mission has no vehicle_id";
    }
...
// to reuse (this process never uploaded anything to this vehicle this
...
// MAVSDK only offers operator<< for Mission::Result (see VehicleActions::result_name
// for the same fmt 8 vs 9 note: fmt::streamed needs fmt 9, Ubuntu 22.04's spdlog
// bundles fmt 8).
std::string VehicleMission::result_name(const mavsdk::Mission::Result result) {
    std::ostringstream stream;
    stream << result;
    return stream.str();
}

VehicleMission::VehicleMission(VehicleConnection& connection)
...
bool VehicleMission::ensure_mission() const {
    ...
    // call it), so `this` here is const VehicleMission*; handle_progress()
    ...
    progress_handle_ = mission_->subscribe_mission_progress(
        [self = const_cast<VehicleMission*>(this)](const mavsdk::Mission::MissionProgress progress) {
...
mavsdk::Mission::Result VehicleMission::log_result(const std::string& label,
...
mavsdk::Mission::MissionPlan VehicleMission::translate(const karshipta::v1::Mission& mission,
                                                        bool& ends_with_rtl) {
    // Precondition: validate_mission() has already accepted `mission` (non-empty
    // vehicle_id, at least one item, RTL only ever at the last index). This
...
karshipta::v1::Mission VehicleMission::translate_back(const mavsdk::Mission::MissionPlan& plan) {
...
void VehicleMission::run_worker(const std::stop_token& stop_token) {
    while (true) {
        Job job;
        {
        ...
        if (const auto rejection = validate_mission(job.mission)) {
            spdlog::error("mission upload rejected for {}: {}", job.mission.vehicle_id(), *rejection);
...

        {
            ...
            if (result == mavsdk::Mission::Result::Success) {
                ...
                interrupted_ = false;
                last_progress_.set_vehicle_id(job.mission.vehicle_id());
...
void VehicleMission::handle_progress(const mavsdk::Mission::MissionProgress progress) {
    ...
    if (passes_remaining_ > 0) {
        ...
        mission_->start_mission_async([](const mavsdk::Mission::Result result) {
            if (result != mavsdk::Mission::Result::Success) {
                spdlog::error("mission re-trigger failed: {}", VehicleMission::result_name(result));
            }
        });
        return;
    }
...
karshipta::v1::MissionProgress VehicleMission::get_progress() const {
    std::lock_guard lock(state_mutex_);
    return last_progress_;
}

bool VehicleMission::notify_interrupted() {
    std::lock_guard lock(state_mutex_);
...
bool VehicleMission::take_pending_return_to_launch() {
    std::lock_guard lock(state_mutex_);
...
std::optional<VehicleMission::UploadResult> VehicleMission::take_upload_result() {
    std::lock_guard lock(state_mutex_);
...
std::optional<VehicleMission::DownloadResult> VehicleMission::take_download_result() {
    std::lock_guard lock(state_mutex_);
