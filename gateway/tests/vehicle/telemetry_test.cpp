...
// TelemetryInfo against a never-connected vehicle (no MAVSDK round trip
...
// vehicle_connection_test.cpp's make_fake_autopilot): subscribe/unsubscribe
...
// autopilot, enough for VehicleConnection::connect() to succeed and for a
...
TEST(TelemetryInfo, GettersReturnSafeDefaultsWithoutConnection) {
    auto core = VehicleConnection::create_shared_core();
    VehicleConnection vehicle(core, listen_url(kBasePort + 1));
    TelemetryInfo telemetry(vehicle);
...
TEST(TelemetryInfo, DestructorSafeWithoutConnection) {
    auto core = VehicleConnection::create_shared_core();
    VehicleConnection vehicle(core, listen_url(kBasePort + 2));
    EXPECT_NO_THROW({ TelemetryInfo telemetry(vehicle); });
}
...
TEST(TelemetryInfo, DestructorUnsubscribesActivePositionSubscription) {
    ...
    auto core = VehicleConnection::create_shared_core();
    VehicleConnection vehicle(core, listen_url(port), /*expected_system_id=*/2);
    ASSERT_EQ(vehicle.connect(), VehicleConnection::ConnectResult::kSuccess);
