...
- No backend server and no database on the console side; the gateway persists its own state locally (YAML for ward configs, SQLite for Fleet/Zone/Fleet Mission) and talks to the console directly.
...
Gateway milestones M1 to M10 are specified in `gateway/BRIEF.md`: MAVSDK connectivity through commands, missions, multi-ward, Herald ingestion, relay transport, Fleet/Zone/Fleet Mission, and concurrency hardening. Console milestones C1 to C8 are in `ROADMAP.md`.
