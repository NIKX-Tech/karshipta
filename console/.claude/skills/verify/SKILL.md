...
- **Empty state**: on load, the map shows "No wards yet" with three actions (Add demo ward / Add simulated ward / Connect real ward); FLEET count reads 0; top bar status button reads DOWN.
...
- **Gateway connection**: clicking the top bar status button opens the connection panel (prefilled `ws://localhost:8765`); Connect shows CONNECTING then LIVE against a running gateway, or stays DOWN retrying with backoff against nothing. Disconnect removes only `source: gateway` wards, demo wards stay.
