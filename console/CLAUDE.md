...
You are assisting development of the Karshipta console: a SvelteKit web dashboard showing live vehicle fleets on a map and sending commands and missions to the gateway.
...
3. State architecture: one `FleetStore` module (Svelte 5 runes in a .svelte.ts file) owns all vehicle state, keyed by vehicle_id. Components subscribe to the store; nothing else touches the WebSocket.
