import type { Airport, AirportCategory, ViewportBounds } from './types';
...
// only, exported via this package's own "./airports-proxy" subpath -
...
// Same constraint as obstacles/openaipProxy.ts's own MAX_BBOX_SPAN_DEG.
...
// OpenAIP airport `type` codes, per their public API docs - same caveat
...
// Same { value, unit } shape confirmed live on obstacles' own elevation field.
...
/** Fetches and parses airports within bounds from OpenAIP. Server-side
 * only (see this file's own header comment) - a consuming app's own
 * same-origin API route calls this and hands the already-typed Airport[]
 * back to the browser, e.g.:
 *
 * // src/routes/api/airports/+server.ts
 * import { fetchAirportsInBounds } from
 *   '@nikx-tech/karshipta-console-core/airports-proxy';
 * export const GET = async ({ url }) => {
 *   const bbox = url.searchParams.get('bbox')?.split(',').map(Number);
 *   return json(await fetchAirportsInBounds(bbox, env.OPENAIP_KEY));
 * };
 */
