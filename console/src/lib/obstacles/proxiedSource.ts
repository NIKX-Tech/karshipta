 ...
 * pointless. obstacle-store.svelte.ts checks for this specifically to stop
 * its own retry loop. */
export class ObstacleProxyAccessError extends Error {}

/** Calls this app's own same-origin obstacles proxy route rather than
 ...
 * /api/obstacles just gets a 404 here, which fails the same clean way any
 * other network error does - the layer degrades gracefully, it doesn't
 * crash the console. */
export class ProxiedObstacleSource implements ObstacleSource {
	async fetchViewport(bounds: ViewportBounds): Promise<Obstacle[]> {
		const url = `/api/obstacles?bbox=${bounds.join(',')}`;
		const response = await fetch(url);
		if (!response.ok) {
			const message = `obstacles proxy request failed: ${response.status} ${response.statusText}`;
			if (response.status !== 429 && response.status >= 400 && response.status < 500) {
				throw new ObstacleProxyAccessError(message);
			}
